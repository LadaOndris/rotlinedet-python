...
class InfiniteDataLoader:
    ...
    def _generate_line(self, image):
        ...
        return image + tf.cast(line, dtype=image.dtype)

    def _preprocess_image(self, image_path):
        image = self._parse_image(image_path)
        image, label = self._draw_line(image)
        return image, label
...
if __name__ == "__main__":
    ...
    batch_size = 8
    data_loader = InfiniteDataLoader('data/without_laser/train',
                                     stripe_width=150,
